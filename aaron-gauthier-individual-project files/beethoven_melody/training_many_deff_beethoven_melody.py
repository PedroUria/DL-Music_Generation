...
# on many songs by stacking them together sequentially, but
...
def load(author, subgenre, number, time_step=0.25):
    ...
    for i in range(len(songs)):
        try:
            notes = get_right_hand(songs[i], time_step=time_step)  # Encodes both hands of the piece
            encoded_notes.append(torch.from_numpy(notes.reshape(-1, 1, 89)).float().cuda())  # as tensor
...
def ltsm_gen(net, seq_len, file_name, sampling_idx=0, sequence_start=0, n_steps=100, hidden_size=89,
    ...
    for _ in range(n_steps):
        ...
        if _ > 6 and note_stuck:
            if (notes[-1] == notes[-2]).sum(2)[0][0].numpy() in [88, 89]:
                if (notes[-1] == notes[-3]).sum(2)[0][0].numpy() in [88, 89]:
                    if (notes[-1] == notes[-4]).sum(2)[0][0].numpy() in [88, 89]:
                        if (notes[-1] == notes[-5]).sum(2)[0][0].numpy() in [88, 89]:
                            if (notes[-1] == notes[-6]).sum(2)[0][0].numpy() in [88, 89]:
                                for m in range(5):
                                    notes.pop(-1)
                                if sampling_idx >= len(notes_encoded):
                                    sampling_idx = 0
    ...
    gen_midi = decode(get_tempo_dim_back(gen_notes, 74), time_step=time_step)
...
def ltsm_gen_v2(net, seq_len, file_name, sampling_idx=0, note_pos=0, n_steps=100, hidden_size=89,
                time_step=0.05, changing_note=False, note_stuck=False, remove_extra_rests=True):
...
notes_encoded = load("beethoven", "sonata", 4)
net, l, ll = train_lstm_loss_whole_seq(100, hidden_size=89, lr=0.01, n_epochs=100, use_all_seq=True)
torch.save(net.state_dict(), 'lstm_beethoven_melody.pkl')
# net = LSTMMusic(89, 89).cuda()
# net.load_state_dict(torch.load("lstm_bach.pkl"))
# net.eval()
ltsm_gen_v2(net, 100, "beethoven_melody", hidden_size=89, time_step=0.25, n_steps=1000, changing_note=True, note_stuck=True)
